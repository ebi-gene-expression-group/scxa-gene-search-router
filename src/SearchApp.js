...
class SearchApp extends Component {
  ...
  handleSelections(routepath, nextSelectedFacets) {
    const queryURL =
      URI(this.props.history.location.pathname)
        .query(routepath)
        .addQuery(this.state.category,this.state.value)
        .addQuery(`species`, this.state.species)
        .toString()
        .replace(/\%2B/g, `+`)
  ...
  componentWillMount() {
    ...
    const category =
      Object.keys(values)
        .filter(val => val === `symbol` || val === `q` || val === `mgi_symbol` || val === `hgnc_symbol`)[0]
    ...
    this.setState({
      ...
      species: values.species ?
        values.species.replace(` `, `+`) :
        ``,
      value: value,
      routepath: _.isEmpty(values) ?
        `test` :
    ...
    return (
      <div className={`App`}>
        <div className={props.wrapperClassName}>
            <GeneSearchForm {...props}
              currentValue={this.state.value}
              currentSpecies={this.state.species ? this.state.species.replace(`+`, ` `) : this.state.species}
              onChange={this.handleSubmit} speciesSelectOnChange={this.speciesSelectOnChange} />
        </div>

        <SwitchRoute {...props}
          routerEndpoint={routerEndpoint}
          nextSelectedFacets={this.state.nextSelectedFacets}
